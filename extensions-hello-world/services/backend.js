...
function voteHandler(req) {
  // Verify all requests.
  console.log('entered vote handler');
  const payload = verifyAndDecode(req.headers.authorization);
  // get the team_id of the vote
  const params = url.parse(req.url, true).query;
  const team_id = params.team_id
  //const team_id = req.team_id;
  const { channel_id: channelId, opaque_user_id: opaqueUserId } = payload;

  // Update votes, use all push to one votes array at first
  votedb.votes.push({user_id: opaqueUserId, team: team_id});
  console.log(votedb.votes);
